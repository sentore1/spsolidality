          ...
          <div className="space-y-6">
            {sections.filter(s => ['mission', 'vision'].includes(s.section_key)).map((section) => {
              ...
              <div key={section.id} className="bg-white p-6 rounded-lg shadow">
                ...
                <div className="space-y-4">
                  <div className="mb-4">
                    <label className="block font-semibold mb-2">Section Images (shown in card)</label>
                    <input type="file" accept="image/*" onChange={async (e) => {
                      const file = e.target.files?.[0];
                      if (file) {
                        const url = await uploadImage(file);
                        if (url) setEditingItem({...current, media_urls: [...(current.media_urls || []), url]});
                      }
                    }} className="w-full border p-2 rounded mb-2" disabled={uploading} />
                    <input type="text" placeholder="Or paste image URL and press Add" onKeyDown={(e) => {
                      if (e.key === 'Enter') {
                        const val = (e.target as HTMLInputElement).value.trim();
                        if (val) { setEditingItem({...current, media_urls: [...(current.media_urls || []), val]}); (e.target as HTMLInputElement).value = ''; }
                      }
                    }} className="w-full border p-2 rounded mb-2" />
                    <div className="flex flex-wrap gap-2 mt-2">
                      {(current.media_urls || []).map((url: string, i: number) => (
                        <div key={i} className="relative group">
                          <img src={url} alt="" className="w-24 h-16 object-cover rounded border" />
                          <button onClick={() => setEditingItem({...current, media_urls: current.media_urls.filter((_: string, j: number) => j !== i)})} className="absolute top-0 right-0 bg-red-600 text-white text-xs w-5 h-5 flex items-center justify-center opacity-0 group-hover:opacity-100">×</button>
                        </div>
                      ))}
                    </div>
                  </div>
          ...
          <div className="space-y-6">
            {sections.filter(s => s.section_key === 'cta_section').map((section) => {
              ...
              <div key={section.id} className="bg-white p-6 rounded-lg shadow">
                ...
                <div className="space-y-4">
                  <div className="mb-4">
                    <label className="block font-semibold mb-2">Background Image (optional overlay on blue section)</label>
                    <input type="file" accept="image/*" onChange={async (e) => {
                      const file = e.target.files?.[0];
                      if (file) {
                        const url = await uploadImage(file);
                    ...
                    {current.media_urls?.[0] && (
                      <div className="mt-2 flex items-center gap-3">
                        <img src={current.media_urls[0]} alt="Preview" className="w-32 h-20 object-cover rounded border" />
                        <button onClick={() => setEditingItem({...current, media_urls: []})} className="text-red-600 text-sm underline">Remove</button>
                      </div>
                    )}
                  </div>
                  {languages.map(lang => (
